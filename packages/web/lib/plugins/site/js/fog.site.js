$(function () {
  ...
  $(".action-boxes.del").on("submit", function () {
    ...
    for (var i = 0, len = checked.size(); i < len; i++) {
      accesscontrolIDArray[accesscontrolIDArray.length] = checked
        .eq(i)
        .attr("value");
    }
  ...
  $(".action-boxes.host").on("submit", function () {
    ...
    for (var i = 0, len = checked.size(); i < len; i++) {
      accesscontrolruleIDArray[accesscontrolruleIDArray.length] = checked
        .eq(i)
        .attr("value");
    }
    $('input[name="accesscontrolruleIDArray"]').val(
      accesscontrolruleIDArray.join(",")
    );
  });
  ...
  checkboxAssociations(
    ".toggle-checkboxhost:checkbox",
    ".toggle-host:checkbox"
  );
  checkboxAssociations(
    ".toggle-checkboxhostrm:checkbox",
    ".toggle-hostrm:checkbox"
  );
});
