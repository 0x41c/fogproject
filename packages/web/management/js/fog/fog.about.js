(function ($) {
  ...
  $(".kernvers").each(function () {
    ...
    $.ajax({
      ...
      success: function (gdata) {
        ...
        var setter = $(this)
          .parents("div.hidefirst")
          .prev("a")
          .find(".kernversionupdate");
  ...
  $("#bannerimg").on("click", function (e) {
    ...
    $("#uploader")
      .html('<input type="file" name="' + name + '" class="newbanner"/>')
      .find("input")
      .trigger("click");
  });
...
function setTimeoutElement() {
  $(
    'button[type="submit"]:not(#importbtn, #export, #upload, #Rebranding, #install), #menuSet, #hideSet, #exitSet, #advSet, button[name="saveform"], button[name="delform"], #deletecu'
  ).each(function (e) {
    if ($(this).is(":visible")) {
