  ...
  GraphDiskUsageOpts = {
    ...
    legend: {
      ...
      labelFormatter: function (label, series) {
        ...
        for (
          var i = 0;
          series.data[0][1] >= 1024 && i < units.length - 1;
          i++
        ) {
  ...
  $(".fogversion").each(function () {
    ...
    $.ajax({
      ...
      success: function (gdata) {
        if (typeof gdata == null || typeof gdata == "undefined") {
          var returnvalue = "-";
        } else {
...
function setupActivity() {
  ...
  var updateClientCountGraph = function (data) {
    ...
    clientinterval = setTimeout(
      updateClientCount,
      300000 - ((new Date().getTime() - startTime) % 300000)
    );
  };
  updateClientCount();
...
function setupDiskUsage() {
  ...
  $("#graph-diskusage")
    .parents("a")
    .attr("href", "../management/index.php?node=hwinfo&id=" + nodeid);
  updateDiskUsage();
...
function setupImagingHistory() {
  ...
  $('<div class="tooltip-inner" id="graph-30day-tooltip"></div>')
    .css({
      position: "absolute",
      display: "none",
      opacity: 0.8,
  ...
  Graph30Day.css({
    ...
    if (item) {
      ...
      $("#graph-30day-tooltip")
        .html(item.series.label + ": " + y + " on " + date)
        .css({
          top: item.pageY + 5,
          left: item.pageX + 5,
        })
    ...
    function fetchData() {
      ...
      function onDataReceived(series) {
        ...
        $.each(series, function (index, value) {
          ...
          while (
            n - GraphBandwidthData[index].tx[0][0] >
            GraphBandwidthMaxDataPoints * 1000
          ) {
          ...
          GraphData[index] = {
            label:
              GraphBandwidthData[index].name +
              " (" +
              GraphBandwidthData[index].dev +
              ")",
            data: $("#graph-bandwidth-filters-transmit").hasClass("active")
              ? GraphBandwidthData[index].tx
              : GraphBandwidthData[index].rx,
          };
        });
