      ...
      .blur(function (e) {
        ...
        if (searchElement.val() == "") {
          searchElement
            ...
            .val(searchElement.prop("placeholder"));
          if (this.SearchAJAX) {
      ...
      this.SearchAJAX = $.ajax({
        ...
        beforeSend: function () {
          SubmitButton.addClass("searching")
            .find("i")
            .removeClass()
            .addClass("fogsearch fa fa-spinner fa-pulse fa-fw");
        },
        success: function (response) {
          ...
          SubmitButton.removeClass("searching")
            .find("i")
            .removeClass()
            .addClass("fogsearch fa fa-search");
          thead = $("thead", Container);
          tbody = $("tbody", Container);
          LastCount = dataLength;
          if (dataLength > 0) {
            buildHeaderRow(response.headerData, response.attributes);
            buildRow(response.data, response.templates, response.attributes);
          }
          TableCheck();
...
(function ($) {
  ...
  $(".trigger_expand").on("click", function (e) {
    ...
    if (!$(this).hasClass("activeclicked")) {
      $(this)
        .addClass("activeclicked")
        .html('<h4 class="title">Collapse All</h4>');
      all = $(".expand_trigger").not(".activenow");
    } else {
      $(this)
        .removeClass("activeclicked")
        .html('<h4 class="title">Expand All</h4>');
      all = $(".activenow");
    }
    all.each(function () {
      $(this).trigger("click");
  ...
  $(":password")
    ...
    .before(
      '<span class="input-group-addon"><i class="fa fa-eye-slash fogpasswordeye"></i></span>'
    );
  /**
  ...
  function format(icon) {
    ...
    var _icon = $(
      '<i class="fa fa-' +
        icon.element.value.toLowerCase() +
        ' fa-fw">' +
        icon.text +
        "</i>"
    );
...
function setEditFocus() {
  $("input, textarea")
    .not(
      '[type="number"], [type="checkbox"], [name="groupsel"], [name="nodesel"], [name="ulang"], #uname, #upass, .system-search, .search-input, [type="radio"], [readonly], .tablesorter-filter'
    )
    .on("focus", function (e) {
      e.preventDefault();
      field = $(this);
...
function clearADFields() {
  $("#clearAD").on("click", function (e) {
    e.preventDefault();
    clearDoms = $(
      '#adOU[type="text"], #adDomain, #adUsername, #adPassword, #adPasswordLegacy'
    ).val("");
...
function ajaxRun(
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      dialog.setTitle(title).setMessage(msg).setType(type);
      if (data.error) {
        if (authneeded) {
          setTimeout(function () {
        ...
        if (authneeded) {
          $(
            '<form id="' +
              ...
              '" method="post" action="' +
              url +
              '"><input type="hidden" name="fogguiuser" value="' +
              username +
              '"/><input type="hidden" name="fogguipass" value="' +
              password +
              '"/><input type="hidden" name="nojson"/></form>'
          )
            .appendTo("body")
            .submit()
            .remove();
  ...
  AJAXTaskForceRequest = $.ajax({
    ...
    beforeSend: function () {
      $(this)
        .off("click")
        .removeClass()
        .addClass("fa fa-refresh fa-spin fa-fw icon");
    },
    success: function (gdata) {
      ...
      $(this)
        .off("click")
        .removeClass()
        .addClass("fa fa-angle-double-right fa-fw icon");
    },
    error: function () {
 ...
 */
function buildHeaderRow(data, attributes) {
  var rows = [];
...
function buildRow(data, templates, attributes) {
  ...
  $(".toggle-action:checkbox, .toggle-checkboxAction:checkbox").on(
    "change",
    function (e) {
      checkedIDs = getChecked();
...
function TableCheck() {
  ...
  if (
    node == "task" &&
    $.inArray(sub, ["search", "listhosts", "listgroups"]) < 0
  ) {
...
function setupParserInfo() {
  ...
  $.tablesorter.addParser({
    ...
    is: function (s) {
      return s.match(
        new RegExp(/[0-9]+(\.[0-9]+)?\ (iB|KiB|MiB|GiB|TiB|EiB|ZiB|YiB)/)
      );
    },
    format: function (s) {
      if (s.length < 1) return;
      var suf = s.match(new RegExp(/(iB|KiB|MiB|GiB|TiB|EiB|ZiB|YiB)$/));
      if (typeof suf == "null" || typeof suf == "undefined" || suf == null)
        return;
      var num = parseFloat(
        suf.input.match(new RegExp(/^[0-9]+(\.[0-9]+)?/))[0]
      );
      switch (suf[0]) {
        case "iB":
          return num;
...
function checkField(field, min, max) {
  ...
  $.each(v, function (key, vv) {
    ...
    step =
      vvv[1] === "" || vvv[1] === undefined || vvv[1] === null ? 1 : vvv[1];
...
function DeployStuff() {
  ...
  $('input[name="scheduleType"]').on("change", function () {
    ...
    $("form.deploy-container")
      .on("submit", function () {
        if (scheduleType == "single") {
          // Format check
          validateInput = $("#" + scheduleType + "Options > input").removeClass(
            "error"
          );
          ...
          $(".cronOptions > input[name^='scheduleCron']", $(this)).each(
            function () {
...
function resetEncData(type, typeID) {
  $("#resetSecData").on("click", function () {
    ...
    BootstrapDialog.show({
      ...
      message:
        "Are you sure you wish to reset this " + type + " encryption data?",
      buttons: [
        {
          label: "Yes",
          cssClass: "btn-warning",
          action: function (dialogItself) {
            $.post("../management/index.php?sub=clearAES", postdata);
            dialogItself.close();
          },
        },
        {
    ...
    switch (this.value) {
      case "hourly":
        $(this)
          .parents(".cronOptions")
          .next()
          .find(".scheduleCronMin")
          .focus()
          .val("0");
        $(this)
          .parents(".cronOptions")
          .next()
          .find(".scheduleCronHour")
          .focus()
          .val("*");
        $(this)
          .parents(".cronOptions")
          .next()
          .find(".scheduleCronDOM")
          .focus()
          .val("*");
        $(this)
          .parents(".cronOptions")
          .next()
          .find(".scheduleCronMonth")
          .focus()
          .val("*");
        $(this)
          .parents(".cronOptions")
          .next()
          .find(".scheduleCronDOW")
          .focus()
          .val("*");
        break;
      case "daily":
