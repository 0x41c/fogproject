(function ($) {
  ...
  $(".imagename-input")
    .on("keyup change blur focus focusout", function (e) {
      var start = this.selectionStart,
        end = this.selectionEnd;
      if (typeof iFileVal === "undefined") return;
      if (iFileVal.length == 0)
        $(".imagefile-input").val(this.value.replace(/[^\w+\/\.-]/g, ""));
      this.setSelectionRange(start, end);
      e.preventDefault();
      form = $(this).parents("form");
      validator = form.validate(validatorOpts);
...
function setTimeoutElement() {
  $("#add, #updategen, #updategroups, #primarysel, #groupdel").each(function (
    e
  ) {
    if ($(this).is(":visible")) {
      form = $(this).parents("form");
